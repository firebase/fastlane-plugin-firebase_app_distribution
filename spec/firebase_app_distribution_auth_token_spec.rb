describe Fastlane::Auth::FirebaseAppDistributionAuthClient do
  ...
  let(:payload) { { "access_token" => "fake_auth_token" } }
  ...
  describe '#fetch_auth_token' do
    it 'uses service credentials for authorization when the path is passed in' do
      expect(auth_helper.fetch_auth_token("google_service_path")).to eq("fake_auth_token")
    end
    ...
    it 'fails if no credentials are passed path is empty' do
    ...
    it 'fails if no credentials are passed path is nil' do
      allow(ENV).to receive(:[]).with("FIREBASE_TOKEN").and_return(nil)
